...
Graph::Graph(int width_, int height_):
    ...
    context(Cairo::Context::create(surface)), width(width_), height(height_),
    mark_size(4.0), border(8.0), label_space(4.0), current_variable(-1) {
  // The background
...
int Graph::define_y(const std::string &name, double min, double max, double r,
                    double g, double b) {
  static const struct {
    double r, g, b;
  } default_colors[] = {
  ...
  if(r < 0) {
    const int n =
        variables.size() % (sizeof default_colors / sizeof default_colors[0]);
...
void Graph::compute_bounds() {
  ...
  bbottom = border + fe.descent + fe.height + label_space + fe.descent
            + fe.height + label_space + mark_size;

  // Left hand Y axis markers --------------------------------------------------

  double max = 0;
  for(map<double, string>::iterator it = variables[0].markers.begin();
      it != variables[0].markers.end(); ++it) {
...
void Graph::draw_axes() {
  ...
  // Markers
  context->get_font_extents(fe);
  for(map<double, string>::iterator it = markers.begin(); it != markers.end();
      ++it) {
    context->get_text_extents(it->second, te);
  ...
  for(map<pair<double, double>, string>::iterator it = ranges.begin();
    ...
    if(shaded) {
      context->set_source_rgb(1.0, 0.95, 0.925);
      context->rectangle(xc(xmin), btop, xc(xmax) - xc(xmin),
                         height - bbottom - btop);
      context->fill();
    }
